...
数组：**相同类型数据的有序集合**，其中每个数据称作一个数组元素，每个数组元素通过一个下标进行访问
...
![未命名文件 (1)](img/%E6%9C%AA%E5%91%BD%E5%90%8D%E6%96%87%E4%BB%B6%20(1).png)
...
- 数组的默认初始化
  ...
  - 数组是引用类型，他的元素相当于累的实例变量，因此数组一经分配空间，其中的每个元素按照实例变量同样的方式被隐式初始化
...
- 数组变量属于引用类型，数组也可以看成是对象，其中的每个元素相当于该对象的成员变量。
...
记录数组一共有几行几列有多少个不同的值，把具有不同值的元素和行列及值记录在一个小规模的数组中，缩小程序规模
